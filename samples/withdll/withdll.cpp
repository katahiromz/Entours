...
int main(int argc, char **argv)
{
    ...
    for (DWORD n = 0; n < nDlls; n++) {
        ...
        ec.nExports = 0;
        EntourEnumerateExports(hDll, &ec, ExportCallback);
        FreeLibrary(hDll);

        if (!ec.fHasOrdinal1) {
            printf("withdll.exe: Error: %s does not export ordinal #1.\n",
                   rpszDllsOut[n]);
            printf("             See help entry EntourCreateProcessWithDllEx in Entours.chm.\n");
            return 9004;
        }
    }
    ...
    if (!EntourCreateProcessWithDllsA(szFullExe[0] ? szFullExe : NULL, szCommand,
                                     NULL, NULL, TRUE, dwFlags, NULL, NULL,
                                     &si, &pi, nDlls, rpszDllsOut, NULL)) {
        DWORD dwError = GetLastError();
        printf("withdll.exe: EntourCreateProcessWithDllEx failed: %d\n", (int)dwError);
        if (dwError == ERROR_INVALID_HANDLE) {
#if ENTOURS_64BIT
            printf("withdll.exe: Can't entour a 32-bit target process from a 64-bit parent process.\n");
#else
            printf("withdll.exe: Can't entour a 64-bit target process from a 32-bit parent process.\n");
#endif
        }
        ExitProcess(9009);
    }
