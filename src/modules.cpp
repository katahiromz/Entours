// Module Enumeration Functions (modules.cpp of entours.lib)
...
const GUID ENTOUR_EXE_RESTORE_GUID = {
...
{
    static ENTOUR_SYM_INFO symInfo;
    static PENTOUR_SYM_INFO pSymInfo = NULL;
...
PVOID WINAPI EntourFindFunction(_In_ PCSTR pszModule,
                                _In_ PCSTR pszFunction)
{
    ...
    ////////////////////////////////////////////////////// Then try ImageHelp.
    ENTOUR_TRACE(("EntourFindFunction(%hs, %hs)\n", pszModule, pszFunction));
    PENTOUR_SYM_INFO pSymInfo = EntourLoadImageHlp();
    if (pSymInfo == NULL) {
        ENTOUR_TRACE(("EntourLoadImageHlp failed: %d\n",
                      GetLastError()));
        return NULL;
    }
    ...
    if (pSymInfo->pfSymLoadModule64(pSymInfo->hProcess, NULL,
        ...
        if (ERROR_SUCCESS != GetLastError()) {
            ENTOUR_TRACE(("SymLoadModule64(%p) failed: %d\n",
    ...
    if (!pSymInfo->pfSymGetModuleInfo64(pSymInfo->hProcess, (DWORD64)hModule, &modinfo)) {
        ENTOUR_TRACE(("SymGetModuleInfo64(%p, %p) failed: %d\n",
    ...
    if (FAILED(hrRet)) {
        ENTOUR_TRACE(("StringCchCopyA failed: %08x\n", hrRet));
        return NULL;
    }
    hrRet = StringCchCatA(szFullName, sizeof(szFullName)/sizeof(CHAR), "!");
    if (FAILED(hrRet)) {
        ENTOUR_TRACE(("StringCchCatA failed: %08x\n", hrRet));
        return NULL;
    }
    ...
    if (!pSymInfo->pfSymFromName(pSymInfo->hProcess, szFullName, &symbol)) {
        ENTOUR_TRACE(("SymFromName(%hs) failed: %d\n", szFullName, (int)GetLastError()));
        return NULL;
    }
    ...
    PPLABEL_DESCRIPTOR pldEntry = (PPLABEL_DESCRIPTOR)EntourGetEntryPoint(hModule);
...
PVOID WINAPI EntourGetEntryPoint(_In_opt_ HMODULE hModule)
{
    ...
    __try {
        ...
        PENTOUR_CLR_HEADER pClrHeader = NULL;
                                     ...
                                     _In_opt_ PF_ENTOUR_IMPORT_FUNC_CALLBACK_EX pfImportFunc)
{
    ...
    __try {
        ...
        for (; iidp->OriginalFirstThunk != 0; iidp++) {
            ...
            HMODULE hFile = EntourGetContainingModule(pAddrs[0]);
...
// Callback for EntourEnumerateImportsEx that adapts EntourEnumerateImportsEx
// for use with a EntourEnumerateImports callback -- derefence the IAT and pass the value on.
static
BOOL
CALLBACK
EntourEnumerateImportsThunk(_In_ PVOID VoidContext,
                            _In_ DWORD nOrdinal,
                            _In_opt_ PCSTR pszFunc,
                            _In_opt_ PVOID* ppvFunc)
{
    _ENTOUR_ENUMERATE_IMPORTS_THUNK_CONTEXT const * const
        pContext = (_ENTOUR_ENUMERATE_IMPORTS_THUNK_CONTEXT*)VoidContext;
...
{
    _ENTOUR_ENUMERATE_IMPORTS_THUNK_CONTEXT const context = { pContext, pfImportFunc };

    return EntourEnumerateImportsEx(hModule,
                                    (PVOID)&context,
                                    pfImportFile,
                                    &EntourEnumerateImportsThunk);
}

static PENTOUR_LOADED_BINARY WINAPI GetPayloadSectionFromModule(HMODULE hModule)
{
    PIMAGE_DOS_HEADER pDosHeader = (PIMAGE_DOS_HEADER)hModule;
    if (hModule == NULL) {
        pDosHeader = (PIMAGE_DOS_HEADER)GetModuleHandleW(NULL);
    ...
    __try {
        ...
        for (DWORD n = 0; n < pNtHeader->FileHeader.NumberOfSections; n++) {
            if (strcmp((PCHAR)pSectionHeaders[n].Name, ".entour") == 0) {
                ...
                ENTOUR_SECTION_HEADER *pHeader = (ENTOUR_SECTION_HEADER *)pbData;
                if (pHeader->cbHeaderSize < sizeof(ENTOUR_SECTION_HEADER) ||
                    pHeader->nSignature != ENTOUR_SECTION_HEADER_SIGNATURE) {
...
{
    PENTOUR_LOADED_BINARY pBinary = GetPayloadSectionFromModule(hModule);
    if (pBinary == NULL) {
    ...
    __try {
        ENTOUR_SECTION_HEADER *pHeader = (ENTOUR_SECTION_HEADER *)pBinary;
        if (pHeader->cbHeaderSize < sizeof(ENTOUR_SECTION_HEADER) ||
            pHeader->nSignature != ENTOUR_SECTION_HEADER_SIGNATURE) {
                               ...
                               _Out_ DWORD *pcbData)
{
    ...
    __try {
        ...
        for (pbData = pbBeg; pbData < pbEnd;) {
            ENTOUR_SECTION_RECORD *pSection = (ENTOUR_SECTION_RECORD *)pbData;
...
PVOID WINAPI EntourFindPayloadEx(_In_ REFGUID rguid,
                                 _Out_ DWORD * pcbData)
{
    for (HMODULE hMod = NULL; (hMod = EntourEnumerateModules(hMod)) != NULL;) {
        PVOID pvData;

        pvData = EntourFindPayload(hMod, rguid, pcbData);
        if (pvData != NULL) {
            return pvData;
        }
    }
...
BOOL WINAPI EntourRestoreAfterWithEx(_In_reads_bytes_(cbData) PVOID pvData,
                                     _In_ DWORD cbData)
{
    PENTOUR_EXE_RESTORE pder = (PENTOUR_EXE_RESTORE)pvData;
    ...
    if (pder->pclr != NULL && pder->clr.Flags != ((PENTOUR_CLR_HEADER)pder->pclr)->Flags) {
        // If we had to promote the 32/64-bit agnostic IL to 64-bit, we can't restore
        // that.
        fUpdated32To64 = TRUE;
    }

    if (EntourVirtualProtectSameExecute(pder->pidh, pder->cbidh,
                                        PAGE_EXECUTE_READWRITE, &dwPermIdh)) {
        if (EntourVirtualProtectSameExecute(pder->pinh, pder->cbinh,
            ...
            if (pder->pclr != NULL && !fUpdated32To64) {
                if (EntourVirtualProtectSameExecute(pder->pclr, pder->cbclr,
...
BOOL WINAPI EntourRestoreAfterWith()
{
    PVOID pvData;
    DWORD cbData;

    pvData = EntourFindPayloadEx(ENTOUR_EXE_RESTORE_GUID, &cbData);

    if (pvData != NULL && cbData != 0) {
        return EntourRestoreAfterWithEx(pvData, cbData);
    }
    SetLastError(ERROR_MOD_NOT_FOUND);
    return FALSE;
}
