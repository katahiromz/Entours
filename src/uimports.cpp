// Add DLLs to a module import table (uimports.cpp of entours.lib)
                              ...
                              DWORD nDlls)
{
    ...
    if (inh.OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR_MAGIC_XX) {
        ENTOUR_TRACE(("Wrong size image (%04x != %04x).\n",
    ...
    for (i = 0; i < inh.FileHeader.NumberOfSections; i++) {
        ...
        ENTOUR_TRACE(("ish[%d] : va=%08x sr=%d\n", i, ish.VirtualAddress, ish.SizeOfRawData));
    ...
    if (pbNew == NULL) {
        ENTOUR_TRACE(("new BYTE [cbNew] failed.\n"));
        goto finish;
    }
    ...
    if (inh.IMPORT_DIRECTORY.VirtualAddress != 0) {
        ...
        ENTOUR_TRACE(("IMPORT_DIRECTORY perms=%x\n", dwProtect));
        ...
        if (!ReadProcessMemory(hProcess,
            ...
            ENTOUR_TRACE(("ReadProcessMemory(imports) failed: %d\n", (int)GetLastError()));
            goto finish;
        }
    }
    ...
    for (n = 0; n < nDlls; n++) {
        ...
        if (FAILED(hrRet)) {
            ENTOUR_TRACE(("StringCchCopyA failed: %d\n", (int)GetLastError()));
            goto finish;
        }
        ...
        hrRet = ReplaceOptionalSizeA((char*)pbNew + obStr,
                                     cbNew - obStr,
                                     ENTOURS_STRINGIFY(ENTOURS_BITS_XX));
        if (FAILED(hrRet)) {
            ENTOUR_TRACE(("ReplaceOptionalSizeA failed: %d\n", (int)GetLastError()));
            goto finish;
        }
    ...
    for (i = 0; i < nDlls + nOldDlls; i++) {
        ENTOUR_TRACE(("%8d. Look=%08x Time=%08x Fore=%08x Name=%08x Addr=%08x\n",
                      i,
    ...
    if (!WriteProcessMemory(hProcess, pbNewIid, pbNew, obStr, NULL)) {
        ENTOUR_TRACE(("WriteProcessMemory(iid) failed: %d\n", (int)GetLastError()));
        goto finish;
    }

    ENTOUR_TRACE(("obBaseBef = %08x..%08x\n",
                  inh.IMPORT_DIRECTORY.VirtualAddress,
                  inh.IMPORT_DIRECTORY.VirtualAddress + inh.IMPORT_DIRECTORY.Size));
    ENTOUR_TRACE(("obBaseAft = %08x..%08x\n", obBase, obBase + obStr));
    ...
    if (!EntourVirtualProtectSameExecuteEx(hProcess, pbModule, inh.OptionalHeader.SizeOfHeaders,
                                           PAGE_EXECUTE_READWRITE, &dwProtect)) {
        ENTOUR_TRACE(("VirtualProtectEx(inh) write failed: %d\n", (int)GetLastError()));
        goto finish;
    }
    ...
    ENTOUR_TRACE(("WriteProcessMemory(idh:%p..%p)\n", pbModule, pbModule + sizeof(idh)));

    if (!WriteProcessMemory(hProcess, pbModule + idh.e_lfanew, &inh, sizeof(inh), NULL)) {
        ENTOUR_TRACE(("WriteProcessMemory(inh) failed: %d\n", (int)GetLastError()));
        goto finish;
    }
    ...
    if (!VirtualProtectEx(hProcess, pbModule, inh.OptionalHeader.SizeOfHeaders,
                          dwProtect, &dwProtect)) {
        ENTOUR_TRACE(("VirtualProtectEx(idh) restore failed: %d\n", (int)GetLastError()));
        goto finish;
    }
