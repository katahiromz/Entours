// Image manipulation functions (image.cpp of entours.lib)
...
#endif  // ndef _STRSAFE_H_INCLUDED_
...
public:
    ...
    static CImage *         IsValid(PENTOUR_BINARY pBinary);
...
public:                                                 // Manipulation Functions
    ...
    BOOL                    EditImports(LPCVOID pContext,
                                        ...
                                        PF_ENTOUR_BINARY_COMMIT_CALLBACK pfCommitCallback);
...
private:
    ...
    BOOL                    m_fHadEntourSection;

private:
    enum {
        ENTOUR_IMAGE_VALID_SIGNATURE = 0xfedcba01,      // "Dtr\0"
    };
};
...
BOOL CImageData::IsValid()
{
    ...
    for (PBYTE pbIter = pbBeg; pbIter < pbEnd;) {
        PENTOUR_SECTION_RECORD pRecord = (PENTOUR_SECTION_RECORD)pbIter;

        if (pRecord->cbBytes < sizeof(ENTOUR_SECTION_RECORD)) {
            return FALSE;
        }
...
PBYTE CImageData::Enumerate(GUID *pGuid, DWORD *pcbData, DWORD *pnIterator)
{
    ...
    if (pnIterator == NULL ||
        m_cbData < *pnIterator + sizeof(ENTOUR_SECTION_RECORD)) {
    ...
    PENTOUR_SECTION_RECORD pRecord = (PENTOUR_SECTION_RECORD)(m_pbData + *pnIterator);

    if (pGuid) {
        *pGuid = pRecord->guid;
    }
    if (pcbData) {
        *pcbData = pRecord->cbBytes - sizeof(ENTOUR_SECTION_RECORD);
    }
...
PBYTE CImageData::Find(REFGUID rguid, DWORD *pcbData)
{
    ...
    DWORD cbBytes = sizeof(ENTOUR_SECTION_RECORD);
    for (DWORD nOffset = 0; nOffset < m_cbData; nOffset += cbBytes) {
        PENTOUR_SECTION_RECORD pRecord = (PENTOUR_SECTION_RECORD)(m_pbData + nOffset);

        cbBytes = pRecord->cbBytes;
        if (cbBytes > m_cbData) {
            break;
        }
        if (cbBytes < sizeof(ENTOUR_SECTION_RECORD)) {
            continue;
        }
        ...
        if (pRecord->guid.Data1 == rguid.Data1 &&
            ...
            *pcbData = cbBytes - sizeof(ENTOUR_SECTION_RECORD);
            return (PBYTE)(pRecord + 1);
        }
    }
...
BOOL CImageData::Delete(REFGUID rguid)
{
    ...
    pbFound -= sizeof(ENTOUR_SECTION_RECORD);
    cbFound += sizeof(ENTOUR_SECTION_RECORD);
...
PBYTE CImageData::Set(REFGUID rguid, PBYTE pbData, DWORD cbData)
{
    ...
    if (!SizeTo(m_cbData + cbAlloc + sizeof(ENTOUR_SECTION_RECORD))) {
        return NULL;
    }

    PENTOUR_SECTION_RECORD pRecord = (PENTOUR_SECTION_RECORD)(m_pbData + m_cbData);
    pRecord->cbBytes = cbAlloc + sizeof(ENTOUR_SECTION_RECORD);
    pRecord->nReserved = 0;
    pRecord->guid = rguid;
    ...
    m_cbData += cbAlloc + sizeof(ENTOUR_SECTION_RECORD);
...
//////////////////////////////////////////////////////////////////////////////
//
CImage * CImage::IsValid(PENTOUR_BINARY pBinary)
{
    if (pBinary) {
        CImage *pImage = (CImage *)pBinary;

        if (pImage->m_dwValidSignature == ENTOUR_IMAGE_VALID_SIGNATURE) {
            return pImage;
        }
    }
...
CImage::CImage()
{
    m_dwValidSignature = (DWORD)ENTOUR_IMAGE_VALID_SIGNATURE;
...
BOOL CImage::Read(HANDLE hFile)
{
    ...
    /////////////////////////////////////////////////// Parse .entour Section.

    DWORD rvaOriginalImageDirectory = 0;
    DWORD rvaEntourBeg = 0;
    DWORD rvaEntourEnd = 0;

    _Analysis_assume_(m_NtHeader.FileHeader.NumberOfSections <= ARRAYSIZE(m_SectionHeaders));

    for (n = 0; n < m_NtHeader.FileHeader.NumberOfSections; n++) {
        if (strcmp((PCHAR)m_SectionHeaders[n].Name, ".entour") == 0) {
            ENTOUR_SECTION_HEADER dh;
            CopyMemory(&dh,
            ...
            rvaEntourBeg = m_SectionHeaders[n].VirtualAddress;
            rvaEntourEnd = rvaEntourBeg + m_SectionHeaders[n].SizeOfRawData;
        }
    }
    ...
    for (n = 0; n < nFiles; n++, iidp++) {
        ...
        *ppLastFile = pImportFile;
        ...
        if ((ULONG)iidp->FirstThunk >= rvaEntourBeg &&
            (ULONG)iidp->FirstThunk < rvaEntourEnd) {
...
BOOL CImage::CheckImportsNeeded(DWORD *pnTables, DWORD *pnThunks, DWORD *pnChars)
{
    ...
    for (CImageImportFile *pImportFile = m_pImportFiles;
        ...
        if (pImportFile->m_fByway) {
            fNeedEntourSection = TRUE;
            nThunks++;
        }
        else {
            if (!fNeedEntourSection &&
...
BOOL CImage::EditImports(LPCVOID pContext,
                         PF_ENTOUR_BINARY_BYWAY_CALLBACK pfBywayCallback,
                         PF_ENTOUR_BINARY_FILE_CALLBACK pfFileCallback,
                         PF_ENTOUR_BINARY_SYMBOL_CALLBACK pfSymbolCallback,
                         PF_ENTOUR_BINARY_COMMIT_CALLBACK pfCommitCallback)
...
BOOL CImage::Write(HANDLE hFile)
{
    ...
    DWORD nThunks = 0;
    DWORD nChars = 0;
    BOOL fNeedEntourSection = CheckImportsNeeded(&nTables, &nThunks, &nChars);
    ...
    if (fNeedEntourSection || !m_pImageData->IsEmpty()) {
    ...
    if (fNeedEntourSection || !m_pImageData->IsEmpty()) {
        ...
        dh.cbHeaderSize = sizeof(ENTOUR_SECTION_HEADER);
        dh.nSignature = ENTOUR_SECTION_HEADER_SIGNATURE;
        ...
        if (clrAddr && clrSize) {
            PENTOUR_CLR_HEADER pHdr = (PENTOUR_CLR_HEADER)RvaToVa(clrAddr);
            if (pHdr != NULL) {
                ENTOUR_CLR_HEADER hdr;
                hdr = *pHdr;
        ...
        HRESULT hrRet = StringCchCopyA((PCHAR)m_SectionHeaders[nSection].Name, IMAGE_SIZEOF_SHORT_NAME , ".entour");
        if (FAILED(hrRet))
            return FALSE;
    ...
    DWORD clrSize = m_NtHeader.OptionalHeader
        .DataDirectory[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR].Size;
    if (clrAddr && clrSize && fNeedEntourSection) {
...
{
    Entour::CImage *pImage = new NOTHROW
        Entour::CImage;
    if (pImage == NULL) {
        SetLastError(ERROR_OUTOFMEMORY);
        return FALSE;
    ...
    return (PENTOUR_BINARY)pImage;
}

BOOL WINAPI EntourBinaryWrite(_In_ PENTOUR_BINARY pdi,
                              _In_ HANDLE hFile)
{
    Entour::CImage *pImage = Entour::CImage::IsValid(pdi);
    if (pImage == NULL) {
        return FALSE;
    }
...
{
    Entour::CImage *pImage = Entour::CImage::IsValid(pBinary);
    if (pImage == NULL) {
        return FALSE;
    }
...
BOOL WINAPI EntourBinaryDeletePayload(_In_ PENTOUR_BINARY pBinary,
                                      _In_ REFGUID rguid)
{
    Entour::CImage *pImage = Entour::CImage::IsValid(pBinary);
    if (pImage == NULL) {
...
static BOOL CALLBACK ResetBywayCallback(_In_opt_ LPCVOID pContext,
...
static BOOL CALLBACK ResetFileCallback(_In_opt_ LPCVOID pContext,
...
static BOOL CALLBACK ResetSymbolCallback(_In_opt_ LPCVOID pContext,
